...
function broadcastOnlineCount(){
  const count = connectedWallets.size;
  const msg = JSON.stringify({type:'online_count', count});
  if (wss) wss.clients.forEach(c => { if (c.readyState === WebSocket.OPEN) c.send(msg); });
}
...
wss.on('connection', (ws) => {
  clients.set(ws, {});
  // Broadcast new online count to all
  ...
  ws.on('message', async (raw) => {
    ...
    if (msg.type === 'player_aim'){
      // Relay aim to opponent only
      const room = rooms.get(msg.roomId);
      if(!room) return;
      const oppIdx = client.playerIdx === 0 ? 1 : 0;
      const opp = room.players[oppIdx];
      if(opp && opp.ws.readyState === WebSocket.OPEN){
        opp.ws.send(JSON.stringify({type:'opponent_aim', angle:msg.angle, color:msg.color}));
      }
      return;
    }

    if (msg.type === 'player_fire'){
      const room = rooms.get(msg.roomId);
      if(!room) return;
      const oppIdx = client.playerIdx === 0 ? 1 : 0;
      const opp = room.players[oppIdx];
      if(opp && opp.ws.readyState === WebSocket.OPEN){
        opp.ws.send(JSON.stringify({type:'opponent_fire', angle:msg.angle, color:msg.color}));
      }
      return;
    }
    ...
    if (msg.type === 'game_chat'){
      const room = rooms.get(msg.roomId);
      if(!room) return;
      const senderIdx = client.playerIdx;
      // Only send to the OTHER player, not back to sender
      room.players.forEach((p, i) => {
        if(i !== senderIdx && p.ws.readyState === WebSocket.OPEN){
    ...
    if (msg.type === 'chat'){
      // Server-side spam protection: 2s per client
      const now = Date.now();
      if (client._lastChat && now - client._lastChat < 2000) return;
      client._lastChat = now;
      const chatMsg = JSON.stringify({
        type: 'chat',
        user: (msg.user || 'Player').slice(0,20),
        text: (msg.text || '').slice(0, 120),
        color: msg.color || '#64748b'
      });
      wss.clients.forEach(c => { if (c.readyState === WebSocket.OPEN) c.send(chatMsg); });
      return;
    }

    if (msg.type === 'get_online'){
      ws.send(JSON.stringify({type:'online_count', count: connectedWallets.size}));
      return;
    }

    if (msg.type === 'get_room_info'){
      const room = rooms.get(msg.roomId);
      if (!room || room.status !== 'waiting'){
        ws.send(JSON.stringify({type:'room_info', roomId: msg.roomId, error: 'Room #' + msg.roomId + ' not found or already started'}));
      } else {
        ws.send(JSON.stringify({type:'room_info', roomId: msg.roomId, bet: room.bet}));
      }
      return;
    }
    ...
    if (msg.type === 'wallet_connect'){
      if (msg.walletAddress) {
        connectedWallets.add(msg.walletAddress);
    ...
    if (msg.type === 'wallet_disconnect'){
      if (client.walletAddress) {
        connectedWallets.delete(client.walletAddress);
    ...
    if (msg.type === 'shot'){
      ...
      // Sync grid to opponent
      const oppIdx = idx === 0 ? 1 : 0;
      const opp = room.players[oppIdx];
      if (opp && opp.ws.readyState === WebSocket.OPEN) {
        opp.ws.send(JSON.stringify({
          type: 'opponent_shot',
          points: parseInt(msg.points) || 0,
          grid: msg.grid || null,
          scores: room.scores
        }));
      }
      // Update shooter's score display too
    ...
    if (msg.type === 'turn_done'){
      const room = rooms.get(client.roomId);
      if (!room || room.status !== 'playing') return;
      // Sync grid to all players
      if (msg.grid) room.lastGrid = msg.grid;
      const nextIdx = client.playerIdx === 0 ? 1 : 0;
      room.currentTurn = nextIdx;
      room.players.forEach((p, i) => {
        if (p.ws.readyState === WebSocket.OPEN) {
          p.ws.send(JSON.stringify({
            type: i === nextIdx ? 'your_turn' : 'opponent_turn',
            grid: msg.grid || null
          }));
        }
      });
    ...
    if (msg.type === 'grid_cleared'){
      const room = rooms.get(client.roomId);
      if (!room || room.status !== 'playing') return;
      room.status = 'ended'; // prevent double settle
      const s = room.scores || [0,0];
      let winnerIdx = null;
